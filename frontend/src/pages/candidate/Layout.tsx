...
export default function CandidateLayout() {
    ...
    const { data: passport } = useQuery({ queryKey: ['passport'], queryFn: async () => {
        try { return (await api.get('/candidates/passport')).data; } catch (e) { return null; }
    }});
    
    const { data: roadmap } = useQuery({ queryKey: ['roadmap'], queryFn: async () => {
        try { return (await api.get('/candidates/roadmap')).data; } catch (e) { return null; }
    }});

    const navItems = [
        { to: 'jobs', icon: Briefcase, label: 'Live Job Feed' },
        { to: 'applications', icon: Inbox, label: 'Active Ledgers' },
    ];

    if (passport?.is_active) {
        navItems.push({ to: 'passport', icon: Verified, label: 'Verified Passport' });
    } else if (roadmap) {
        navItems.push({ to: 'roadmap', icon: ChevronRight, label: 'Learn Roadmap' });
    } else {
        navItems.push({ to: 'test', icon: Hexagon, label: 'Skill Assessment' });
    }

    navItems.push(
        { to: 'resume', icon: FileText, label: 'Terminal Parser' },
        { to: 'profile', icon: User, label: 'Profile Ledger' }
    );
        ...
        <div className="min-h-screen bg-fintech-base text-fintech-cream font-body flex flex-col">
                ...
                <nav className="flex-1 flex gap-1 h-full items-end pb-0 overflow-x-auto no-scrollbar">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            `}
                        >
                            <item.icon className={`w-4 h-4 ${item.to === 'passport' && 'text-fintech-sage'}`} />
                ...
                <div className="flex items-center space-x-6 ml-6 shrink-0">
                    ...
                    <div className="flex items-center space-x-4 pl-6 border-l border-fintech-border">
                        <div className="flex flex-col text-right hidden lg:flex">
                            ...
                            <span className="text-[9px] font-terminal text-fintech-sage uppercase tracking-widest mt-0.5">Terminal Active</span>
                        </div>
            ...
            <main className="flex-1 w-full mx-auto px-6 py-12 flex flex-col">
                <div className="flex items-center space-x-3 mb-8 px-6 opacity-60 pointer-events-none">
                    ...
                    <span className="text-[11px] font-terminal uppercase tracking-[0.2em] text-fintech-ash">Candidate Environment // Secure Protocol Active</span>
                </div>
